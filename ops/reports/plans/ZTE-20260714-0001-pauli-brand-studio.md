...
- Risk tier: LOW
- Blast radius: 1 repository, documentation and governance only
...
- No secrets, dependencies, runtime code, package-manager changes, or production configuration are introduced.

## Validation

- Fetch every new file from the branch.
- Compare the branch to `main`.
- Confirm the diff contains only the allowlisted documentation and governance paths.
- Open a pull request with rollback instructions.

## Rollback

Close the pull request without merging, or revert the branch commits. No runtime, data, infrastructure, dependency, or production state changes are involved.
