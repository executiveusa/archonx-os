"""Deployment tools for Coolify-backed deploys and legacy test orchestration."""
...
class DeploymentTool(BaseTool):
    """Primary deployment tool used by runtime orchestration."""
...
class GitHubDeployOrchestrator(BaseTool):
    """Compatibility deployment tool expected by legacy tests and registry wiring."""

    name = "github_deploy_orchestrator"
    description = "Trigger and rollback static-site style deployments"
    ...
    async def execute(self, params: dict[str, Any]) -> ToolResult:
        ...
        if action == "status":
            return ToolResult(
                tool=self.name,
                status="success",
                data={"action": "status", "site_path": site_path, "status": "healthy"},
            )
